...
//
const BotName = 'Azhura Bot'; // Nama Bot Whatsapp
const instagramlu = '@fal.naf'; // Nama Instagramlu cok
const whatsapplu = '0812-9296-8958'; // Nomor whatsapplu cok
const kapanbotaktif = '24 Jam'; // Kapan bot lu aktif
const grupch1 = 'Kosong:v'; // OFFICIAL GRUP LU 1
const grupch2 = 'Kosong:v'; // OFFICIAL GRUP LU 2
//
const
{
   WAConnection,
...
axios.get(`https://mhankbarbar.herokuapp.com/nulis?text=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Semangat ya belajarnya Azhura disini siap membantu\n\n${res.data.result}`;
...
axios.get(`https://mhankbarbar.herokuapp.com/api/yta?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Silahkan download sendiri, takut servernya down hehehe\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/ytv?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Oke silahkan di download sendiri ya, takut servernya down xixi..\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/epbe?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Oke siap sudah Azhura bantu \n\nJudul: ${res.data.title}\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/ig?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Selera kamu bagus juga\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/twit?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Sudah ya kak terimakasih\n\nJudul: ${res.data.title}\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/wiki?q=${teks}&lang=id&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Menurut penelitian azhura (Wikipedia):\n\n${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
conn.sendMessage(id, 'pa pe pa pe sopan dikit sama bot kalau bingung ketik #help ya beb' ,MessageType.text);
}
else if (text == 'P'){
conn.sendMessage(id, 'pa pe pa pe sopan dikit sama bot kalau bingung ketik #help ya beb' ,MessageType.text);
}
else if (text == 'halo'){
conn.sendMessage(id, 'Hai, ada yang bisa Azhura bantu? kalo bingung ketik #help ya sayang..' ,MessageType.text);
}
else if (text == 'hai'){
conn.sendMessage(id, 'Hallo, ada yang bisa saya bantu? kalo bingung ketik #help ya bebs..' ,MessageType.text);
}
else if (text == 'woi'){
conn.sendMessage(id, 'Anjing kasar, ketik #help bos' ,MessageType.text);
}
else if (text == 'woy'){
conn.sendMessage(id, 'Anjing kasar, ketik #help bos' ,MessageType.text);
}
else if (text == 'hi'){
conn.sendMessage(id, 'hai juga, ada yang bisa Azhura bantu ? ketik #help ya' ,MessageType.text);
}
else if (text == 'gan'){
...
conn.sendMessage(id, 'Panggil gw stepen' ,MessageType.text);
}
else if (text == 'Stepen'){
conn.sendMessage(id, 'Oi apaan' ,MessageType.text);
}
else if (text == 'Stev'){
conn.sendMessage(id, 'Iya ada apa?' ,MessageType.text);
}
else if (text == '#donate'){
const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
